...
void PathChecker::ValidatePaths(const std::string& csv_path,
  ...
  if (ec) {
    if (ec == std::errc::no_such_file_or_directory) {
      ...
      throw std::runtime_error("Нет прав доступа к файлу базы хешей: " +
                               csv_path);
    } else {
      throw std::runtime_error("Ошибка доступа к файлу базы хешей: " +
                               csv_path + " (" + ec.message() + ")");
    }
  }
  ...
  if (!test_base.is_open()) {
    throw std::runtime_error("Не удается открыть файл базы хешей для чтения: " +
                             csv_path);
  }
  test_base.close();
  ...
  if (!root_path.empty()) {
    ...
    if (ec) {
      if (ec == std::errc::no_such_file_or_directory) {
        throw std::runtime_error("Директория для сканирования не найдена: " +
                                 root_path);
      } else if (ec == std::errc::permission_denied) {
        throw std::runtime_error(
            "Нет прав доступа к директории сканирования: " + root_path);
      } else {
        throw std::runtime_error("Ошибка доступа к директории сканирования: " +
  ...
  if (!test_log.is_open()) {
    throw std::runtime_error("Не удается открыть файл лога для записи: " +
                             log_path);
  }
  test_log.close();
}

bool PathChecker::IsValidHashBase(const std::string& csv_path) {
  namespace fs = std::filesystem;
  std::error_code ec;
...
bool PathChecker::IsValidLogPath(const std::string& log_path) {
  std::ofstream test_log(log_path, std::ios::app);
  return test_log.is_open();
}

bool PathChecker::IsValidScanDirectory(const std::string& root_path) {
  namespace fs = std::filesystem;
  std::error_code ec;
...
void PathChecker::EnsureLogDirectoryExists(const std::string& log_path) {
  namespace fs = std::filesystem;
